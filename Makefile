...
%.cmo: %.ml
	ocamlfind ocamlc -c -package padiflac -package js_of_ocaml-ppx -package js_of_ocaml $<

%.byte: %.ml
	ocamlbuild -use-ocamlfind -package padiflac -package js_of_ocaml -package js_of_ocaml-ppx $@
...
.PHONY: all clean with-dune with-ocamlbuild
